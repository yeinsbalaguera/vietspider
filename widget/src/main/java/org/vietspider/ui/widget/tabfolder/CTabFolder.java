 ...
 *******************************************************************************/
package org.vietspider.ui.widget.tabfolder;
...
public class CTabFolder extends Composite {
  ...
  static final int[] TOP_LEFT_CORNER = new int[] {0,6, 0,5, 1,4, 4,1, 5,1, 6,0};
  ...
  static final int BACKGROUND = SWT.COLOR_BLUE;//COLOR_WIDGET_BACKGROUND;
  static final int BUTTON_BORDER = SWT.COLOR_WIDGET_DARK_SHADOW;
  static final int BUTTON_FILL = SWT.COLOR_LIST_BACKGROUND;
  
  Color tipFore = new Color(getDisplay(), 25, 20, 20);
  Color tipBack = new Color(getDisplay(), 255, 255, 255);
  ...
  public CTabFolder(Composite parent, int style) {
    ...
    borderColor = display.getSystemColor(SELECTION_BACKGROUND);
    ...
    setTabHeight(22); 
    setFont(UIDATA.FONT_8V);
    setForeground(UIDATA.DISPLAY.getSystemColor(SWT.COLOR_BLACK));
    setSelectionForeground(new Color( this.getDisplay(), 0, 45, 45));
    Color color2 = UIDATA.BCOLOR;
    Color color3 = new Color(getDisplay(), 210, 210, 210);
    Color color4 = new Color(getDisplay(), 240, 240, 240);
    setBackground(color4);    
    setSelectionBackground(new Color[]{ color2, color3},new int[] {70}, true);
    setSimple(false); 
  }
  ...
  void drawMaximize(GC gc) {
    ...
    polyline2 = new int [] {
        ...
        px+3,py+2, px+4,py+2, px+4,py+1,  px+5,py+1, px+5,py, px+6,py, px+6,py-1};
    gc.setForeground(getDisplay().getSystemColor(BUTTON_BORDER));
...
//  }
  ...
  public void setTopRight(Control control, int alignment) {
    checkWidget();
    if (alignment != SWT.RIGHT && alignment != SWT.FILL) SWT.error(SWT.ERROR_INVALID_ARGUMENT);
    
    if (control != null && control.getParent() != this)   SWT.error(SWT.ERROR_INVALID_ARGUMENT);
  ...
  void showToolTip(int x, int y) {
    if (toolTipShell == null) {
      ...
      toolTipLabel.setForeground (tipFore);
      toolTipLabel.setBackground (tipBack);
      for (int i = 0; i < toolTipEvents.length; i++) {
  ...
  boolean updateToolTip (int x, int y) {
    ...
    if(tooltip.equals("Maximize")) tooltip ="Show menu";

...
abstract class TPrimitiveIterator extends TIterator {
  ...
  protected final TPrimitiveHash _hash1;
...
//    if (_expectedSize > _hash1.size()) return -1;
//    if (_expectedSize < _hash1.size())  _expectedSize = _hash1.size();
    if(_expectedSize != _hash1.size()) {
      throw new ConcurrentModificationException();
    }

    byte[] states = _hash1._states;
    int i = _index;

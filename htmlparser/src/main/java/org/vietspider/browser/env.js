...
(function(){
	...
	window.addEventListener = function(type, fn){
		...
		if (!events[this.uuid][type])	events[this.uuid][type] = [];
		
		if (events[this.uuid][type].indexOf( fn ) < 0) events[this.uuid][type].push( fn );
	};
	...
	window.DOMDocument = function(file){
		this._file = file;
		this._dom = Packages.org.vietspider.html.parser.HTMLParser.createDocument(file, null);
		Packages.org.vietspider.js.test.TestJavascriptDOM.doc = this._dom;
		Packages.java.lang.System.out.println("hi hi  "+ this._dom);
	...
	DOMDocument.prototype = {
		...
		getElementsByTagName: function(name){
			return new DOMNodeList( this._dom.getElementsByTagName(name.toLowerCase()));
		},
		...
		getElementById: function(id){
			...
			for ( var i = 0; i < elems.length; i++ ) {
				...
				if ( elem.getAttribute("id") == id ) return makeNode(elem);
			}
		...
		toString: function(){
			return "Document" + (typeof this._file == "string" ? " : " + this._file : "");
		},
	...
	function getDocument(node) {	return obj_nodes.get(node); }
	...
	window.DOMNode = function(node){ this._dom = node; };
	
	DOMNode.prototype = {
		get nodeType() { return this._dom.getNodeType(); },
		
		get nodeValue(){ return this._dom.getNodeValue();	},
		
		get nodeName() { return this._dom.getNodeName(); },
	...
	window.DOMElement = function(elem){
		...
		for ( var i = 0; i < styles.length; i++ ) {
			...
			if ( style.length == 2 ) this.style[ style[0] ] = style[1];
		}
	};
	...
	DOMElement.prototype = extend( new DOMNode(), {
		...
		get tagName(){ return this._dom.getTagName();	},
		...
		get innerHTML() {	return this.childNodes.outerHTML;	},
			...
			if ( this.nodeName == "IFRAME" ) {
				if ( !this._doc )
					this._doc = new DOMDocument(
						...
						"<html><head><title></title></head><body></body></html>")).getBytes("UTF8")));
				return this._doc;
			} else
				return null;
		}
		...
		if ( node ) {
			if ( !obj_nodes.containsKey(node) )
				obj_nodes.put( node, node.getNodeType() == 
					Packages.org.w3c.dom.Node.ELEMENT_NODE ? new DOMElement(node) : new DOMNode(node));
		...
		send: function(data){
			...
			function makeRequest(){
				...
				if (url.getProtocol() == "file" ) {
					...
					var connection = url.openConnection();
					Packages.java.lang.System.out.println("da xay ra url   "+ url);
					...
					for (var i = 0; ; i++) { 
						...
						if (headerName) self.responseHeaders[headerName] = headerValue;
					}
					...
					if (self.responseText.match(/^\s*</) ) {
						try {
							self.responseXML = new DOMDocument(
								new java.io.ByteArrayInputStream((new java.lang.String(self.responseText)).getBytes("UTF8")));
						} catch(e) {
							Packages.java.lang.System.out.println("ta co "+ e);
						}
					}
				}
